    ...
    setup(props, { emit }) {
      ...
      const validateParticipants = () => {
        if (maxParticipants.value <= minParticipants.value) {
          ...
          errorMessage.value = ""; // 유효할 경우 에러 메시지 제거
        }
      };
      watch([maxParticipants, minParticipants], () => {
      ...
      const updatePrice = (value) => {
        ...
        if (!isNaN(numericValue)) {
          price.value = Number(numericValue);
        }
      };
      // Event Type 설정
      ...
      const nextStep = () => {
        ...
        if (errorMessage.value) {
          alert(errorMessage.value); // 활성화된 에러 메시지 표시
          return;
        }
        ...
        // 모든 조건을 만족하면 데이터 전송 및 다음 단계로 이동
        emit("update-form-data", {
          type: type.value,
          question: eventQuestion.value,
          price: price.value,
